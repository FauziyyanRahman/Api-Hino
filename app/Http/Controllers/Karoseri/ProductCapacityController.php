...
use App\Models\Karoseri\ProductCapacity;
...
class ProductCapacityController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($pageSize && $page) {
                ...
                $equipment = ProductCapacity::paginate($pageSize, ['*'], 'page', $page);
            } else {
                // If no pagination parameters are provided, retrieve all records
                $equipment = ProductCapacity::withTrashed()->get();
            }
            ...
            return response()->json([
                'success' => true,
                'message' => 'Production capacity data retrieved successfully.',
                'data' => $equipment,
            ], Response::HTTP_OK);
        } catch (\Exception $e) {
    ...
    {
        $designTool = \DB::select("SELECT * FROM msbodymaker_production_capacity WHERE ms_company_id = $id")[0] ?? null;
        ...
        return response()->json([
            'success' => true,
            'message' => 'Production capacity for company id ' . $id . ' retrieved successfully.',
            'data' => $designTool,
        ], Response::HTTP_OK);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $equipment = ProductCapacity::create($validatedData);
    ...
    public function update(Request $request, $id)
    {
        ...
        $updated = \DB::table('msbodymaker_production_capacity')
    ...
    public function destroy($id)
    {
        ...
        try {
            $designTool = \DB::select("DELETE FROM msbodymaker_production_capacity WHERE ms_company_id = $id");

...
use App\Models\Karoseri\Equipment;
...
class EquipmentController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($pageSize && $page) {
                ...
                $equipment = Equipment::paginate($pageSize, ['*'], 'page', $page);
            } else {
                // If no pagination parameters are provided, retrieve all records
                $equipment = Equipment::withTrashed()->get();
            }
            ...
            return response()->json([
                'success' => true,
                'message' => 'Equipment and tools data retrieved successfully.',
                'data' => $equipment,
            ], Response::HTTP_OK);
        } catch (\Exception $e) {
    ...
    {
        $designTool = \DB::select("SELECT * FROM msbodymaker_equipment WHERE ms_company_id = $id")[0] ?? null;
        ...
        return response()->json([
            'success' => true,
            'message' => 'Equipment and tools data for company id ' . $id . ' retrieved successfully.',
            'data' => $designTool,
        ], Response::HTTP_OK);
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'ms_equipment' => 'required',
            'ms_welding_machine' => 'required',
            'ms_cutting_machine' => 'required',
            'ms_bending_machine' => 'required',
            'ms_plasma_cutting' => 'required',
    ...
    public function update(Request $request, $id)
    {
        ...
        $updated = \DB::table('msbodymaker_equipment')
    ...
    public function destroy($id)
    {
        ...
        try {
            $designTool = \DB::select("DELETE FROM msbodymaker_equipment WHERE ms_company_id = $id");
